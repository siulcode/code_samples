...
# Date: Sept 15 2016
# Customized by: LL.
# DESC: Simple script uploading a file to S3. Supports AWS signature version 4, custom
#       region, permissions and mime-types. Uses Content-MD5 header to guarantee
#       uncorrupted file transfer.
# Note: Make sure you add your credentials on lines 62 & 63 before running the script.
#############################
usage()
{
    cat <<USAGE
...
Examples:
  ...
  'eu-central-1' with key (path relative to bucket) 'media/image.png':
    ...
    `basename $0` '' '' storage@eu-central-1 \\
...
# Inputs.
aws_ak="<YOUR-ACCESSKEY>"                                                # access key
aws_sk="<YOUR-SECRETKEY>"                                                # secret key
